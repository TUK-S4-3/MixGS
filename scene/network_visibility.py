...
#기존 - visible globa 하나당 detail 하나만 출력
...
#visible global 하나당 detail 여러개 출력하도록 수정 -> detail 개수는 allocate_detail_budget 함수로 할당
#solt embedding +shared head 방식 사용
class GSDecoder(nn.Module):
    def __init__(self, spatial_in_dim, mlp_in_dim, depth=1, width=256, max_details=4):
        super().__init__()
        self.depth = depth
        self.width = width
        ...
        self.slot_embedding = nn.Parameter(torch.randn(max_details, width) * 0.01)
    ...
    def forward(self, spatial_h, pose_input, scale_input, rotate_input):
        ...
        cat_feat = self.mlp(cat_feat)
        h = spatial_h * (2 * torch.sigmoid(cat_feat) - 1)
        h = self.tiny_mlp(h)  # [V, W]

        slot_h = h.unsqueeze(1) + self.slot_embedding.unsqueeze(0)  # [V, K, W]
        slot_h = self.slot_mlp(slot_h)

        color = self.gaussian_color(slot_h)        # [V, K, 3]
        scaling = self.gaussian_scaling(slot_h)    # [V, K, 3]
        rotation = self.gaussian_rotation(slot_h)  # [V, K, 4]
        opacity = torch.sigmoid(self.gaussian_opacity(slot_h))  # [V, K, 1]

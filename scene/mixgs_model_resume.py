...
from scene.network_visibility import GSDecoder, GSEncoder #scene.network -> scene.network_visibility로 수정
import os
...
class MixGSModel:
    ...
    def capture(self, include_optimizer=True):
        ...
        if include_optimizer and hasattr(self, "optimizer") and self.optimizer is not None:
            state["optimizer"] = self.optimizer.state_dict()
        return state

    def restore(self, state, load_optimizer=True, strict=True):
        if "encoder" in state:
            self.encoder.load_state_dict(state["encoder"], strict=strict)
        elif "grid" in state:
            self.encoder.load_state_dict(state["grid"], strict=strict)

        if "decoder" in state:
            self.decoder.load_state_dict(state["decoder"], strict=strict)
        elif "network" in state:
            self.decoder.load_state_dict(state["network"], strict=strict)

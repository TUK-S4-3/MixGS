...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, n_offsets : int = 1):
        self.opacity_thr = 0.5
        self.n_offsets = n_offsets      #self.n_offsets = 1 -> self.n_offsets = n_offsets, yaml에서 n_offsets 설정
    ...
    @property
    def get_offset(self):
        return self._offset # return.self._offset.squeeze() -> return.self._offset
        #[N,K,3] 형태로 변환, N : 전역 가우시안 개수, K : 한 전역 가우시안이 가진 detail slot 개수, 3 : offset의 3차원 벡터 값
        #기존 코드로 할 경우 K=1인 상황에서 shape이 [N,3]이 되어버려서 이후 연산에서 shape 관련 오류 발생
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        offsets = torch.zeros((fused_point_cloud.shape[0], self.n_offsets, 3), device="cuda") #offsets = torch.zeros((fused_point_cloud.shape[0], self.n_offsets, 3)).float().cuda() -> offsets = torch.zeros((fused_point_cloud.shape[0], self.n_offsets, 3), device="cuda")
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))

        if self.n_offsets > 1 :  #n_offsets가 1보다 큰 경우에만 offset이 optimizable하게 설정되도록 수정
            base_std = torch.exp(scales).mean(dim=1, keepdim=True).unsqueeze(1)
            offsets[ :, 1:, : ]=0.01 * base_std * torch.randn_like(offsets[ :, 1:, : ])
        self._offset = nn.Parameter(offsets.requires_grad_(True)) 
        #첫 슬롯은 0, 나머지 작은 jitter로 초기화 -> n_offsets가 1인 경우에도 offset이 optimizable하게 설정되도록 수정, n_offsets가 1인 경우에는 offset이 모두 0으로 초기화됨
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] in ("xyz", "offset"):    # if param_group["name"] == "xyz" : -> if param_group["name"] == ("xyz", "offset") : xyz만 decay에서 detail 위치를 _offset이 직접 담당하도록 수정
                param_group['lr'] = lr 
        return lr
    ...
    def save_ply(self, path):
        ...
        offset = self._offset.detach().flatten(start_dim=1).contiguous().cpu().numpy() # [N, K, 3] 형태로 저장, 기존 코드에서는 [N, 3*K] 형태로 저장되어서 ply 파일에서 offset 정보가 제대로 복원되지 않는 문제 발생
    ...
    def load_ply(self, path):
        ...
        # offset property 이름들을 전부 다 읽음
        offset_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_offset_")]
        offset_names = sorted(offset_names, key = lambda x: int(x.split('_')[-1]))

        offsets = np.zeros((xyz.shape[0], self.n_offsets, 3), dtype=np.float32)

        for idx, attr_name in enumerate(offset_names[:self.n_offsets * 3]):     # [N, K, 3] 형태로 복원 
            slot =idx // 3
            ch = idx % 3
            offsets[:, slot, ch] = np.asarray(plydata.elements[0][attr_name])

        opa_flag = expit(opacities).squeeze() > 0.0
        offsets = offsets[opa_flag]

        self._offset = nn.Parameter(torch.tensor(offsets, dtype=torch.float, device="cuda"))
